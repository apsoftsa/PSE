                        ...
                        foreach (IGrouping<string, POS> subCategoryItems in posItemsGroupBySubCat) {
                            switch ((PositionClassifications)int.Parse(subCategoryItems.Key)) {
                                case PositionClassifications.METALS_ACCOUNTS_FUNDS_PHYSICAL: {
                                        foreach (POS posItem in subCategoryItems.Where(f => f.Category_11.Trim().EndsWith("MF")).OrderBy(ob => ob.NumSecurity_29)) {
                                            // Metal funds
                                        ...
                                        foreach (POS posItem in subCategoryItems.Where(f => f.Category_11.Trim().EndsWith("MF") == false).OrderByDescending(ob => ob.Amount1Base_23)) {
                                            if (string.IsNullOrEmpty(posItem.Category_11) == false && posItem.Category_11.Trim().EndsWith("ME")) { //   Physical metals
