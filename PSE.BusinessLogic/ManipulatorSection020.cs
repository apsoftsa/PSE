            ...
            if (extractedData.Any(flt => flt.RecordType == nameof(IDE)) && extractedData.Any(flt => flt.RecordType == nameof(PER)))
            {
                ...
                foreach (IDE ideItem in ideItems)
                {
                    if (perItems != null && perItems.Any(flt => flt.CustomerNumber_2 == ideItem.CustomerNumber_2))
                    {
                        ...
                        {
                            if (perItem.StartDate_6.HasValue && perItem.EndDate_7.HasValue
                            ...
                            tmpInpOut += perItem.SecOut_13.HasValue ? perItem.SecOut_13.Value : 0;
                            historyEvo = new PerformanceEvolutionHistoryCHF()
                            {
                                Period = tmpPeriod,
                                PercentPerformance = AssignRequiredDecimal(perItem.TWR_14),
                                InitialAmount = AssignRequiredDecimal(perItem.StartValue_8),
                                FinalAmount = AssignRequiredDecimal(perItem.EndValue_9),
